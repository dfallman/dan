...
pub(crate) fn char_idx_for_visual_col<I: IntoIterator<Item = char>>(
	chars: I,
	line_len: usize,
	row_start: usize,
	row_end: usize,
...
mod tests {
	...
	#[test]
	fn ascii_visual_col() {
		assert_eq!(visual_col_at("hello".chars(), 0, 4), 0);
		assert_eq!(visual_col_at("hello".chars(), 3, 4), 3);
		assert_eq!(visual_col_at("hello".chars(), 5, 4), 5);
	}
	...
	fn tab_visual_col() {
		...
		assert_eq!(visual_col_at("\thello".chars(), 0, 4), 0);
		assert_eq!(visual_col_at("\thello".chars(), 1, 4), 4); // past the tab
		assert_eq!(visual_col_at("\thello".chars(), 2, 4), 5); // 'e'
	}

	#[test]
	fn cjk_visual_col() {
		// "ab龙cd": a=1, b=1, 龙=2, c=1, d=1
		assert_eq!(visual_col_at("ab龙cd".chars(), 0, 4), 0);
		assert_eq!(visual_col_at("ab龙cd".chars(), 2, 4), 2); // before 龙
		assert_eq!(visual_col_at("ab龙cd".chars(), 3, 4), 4); // after 龙
		assert_eq!(visual_col_at("ab龙cd".chars(), 4, 4), 5); // after c
	}

	#[test]
	fn mid_tab_visual_col() {
		// "a\tb": 'a' at vcol 0..1, tab at vcol 1..4, 'b' at vcol 4
		assert_eq!(visual_col_at("a\tb".chars(), 1, 4), 1); // at the tab char
		assert_eq!(visual_col_at("a\tb".chars(), 2, 4), 4); // past the tab
	}
	...
	fn sticky_column_roundtrip() {
		...
		let start_vcol = visual_col_at(long.chars(), 15, tab_w);

		// Move down to short line — clamps
		let short_col = char_idx_for_visual_col(short.chars(), 2, 0, 2, start_vcol, tab_w, true);
