...
pub fn get_swap_path(original_path: &Path) -> PathBuf {
	if let Some(file_name) = original_path.file_name() {
		...
		if let Some(parent) = local_swp.parent() {
			if fs::metadata(parent).is_ok() && !parent.metadata().map(|m| m.permissions().readonly()).unwrap_or(true) {
				return local_swp;
			}
		}
		...
		let flat_name = original_path.to_string_lossy().replace('/', "_").replace('\\', "_");
...
pub fn write_swap_atomic(swap_path: &Path, content: &str) {
	...
	let tmp_ext = tmp_path.extension().unwrap_or_default().to_string_lossy().into_owned() + ".tmp";
